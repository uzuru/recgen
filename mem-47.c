  ...
  for (int x=0; x<width; x++) {
    for (int y=0; y<width; y++) {
      for (int s=0; s<sub; s++) {
        ...
        int r = (imageData[pos(x, y)+0] < imageData[pos(x, y)+0+width*height*3*s])?imageData[pos(x, y)+0+width*height*3*s]:imageData[pos(x, y)+0];
        int g = (imageData[pos(x, y)+1] < imageData[pos(x, y)+1+width*height*3*s])?imageData[pos(x, y)+0+width*height*3*s]:imageData[pos(x, y)+0];
        int b = (imageData[pos(x, y)+2] < imageData[pos(x, y)+2+width*height*3*s])?imageData[pos(x, y)+0+width*height*3*s]:imageData[pos(x, y)+0];
        if (r>255) r = 255;
        if (g>255) g = 255;
        if (b>255) b = 255;
